...
const SystemDetailModal: React.FC<SystemDetailModalProps> = ({ 
  ...
  const getModalContent = () => {
    switch (type) {
            ...
            <div className="space-y-6">
              ...
              <div className={`rounded-lg p-4 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                <h4 className={`font-semibold mb-3 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                  <FontAwesomeIcon icon={faMicrochip} className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-900'}`} />
                  處理器資訊
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>處理器型號:</span>
                    <div className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{systemHealth.cpu_details?.cpu_brand}</div>
                  </div>
                  <div>
                    <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>核心數量:</span>
                    <div className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{systemHealth.cpu_details?.core_count} 核心</div>
                  </div>
                  <div>
                    <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>基礎頻率:</span>
                    <div className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{systemHealth.cpu_details?.cpu_frequency} MHz</div>
                  </div>
                  <div>
                    <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>總體使用率:</span>
                    <div className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{systemHealth.cpu_details?.cpu_usage.toFixed(1)}%</div>
                  </div>
                </div>
              </div>
              ...
              {systemHealth.cpu_details?.cores && (
                <div className={`rounded-lg p-4 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                  <h4 className={`font-semibold mb-3 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                    <FontAwesomeIcon icon={faTachometerAlt} className={`mr-2 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
                    各核心使用率
                  </h4>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {systemHealth.cpu_details.cores.map((core) => {
                      ...
                      return (
                        <div key={core.core_id} className={`rounded-lg p-3 border border-gray-200 ${isDark ? 'bg-gray-700' : 'bg-white'}`}>
                          <div className="flex justify-between items-center mb-2">
                            <span className={`text-sm font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                              核心 {core.core_id}
                            </span>
                            <span className={`text-xs px-2 py-1 rounded-full ${statusColors.bgColor} ${statusColors.color}`}>
                              {usage.toFixed(1)}%
                            </span>
                          </div>
                          
                          {/* 進度條 */}
                          <div className={`w-full rounded-full h-2 mb-2 ${isDark ? 'bg-gray-500' : 'bg-gray-200'}`}>
                            <div
              ...
              {systemHealth.load_average ? (
                <div className={`rounded-lg p-4 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                  <h4 className={`font-semibold mb-3 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                    <FontAwesomeIcon icon={faWeight} className={`mr-2 ${isDark ? 'text-yellow-400' : 'text-yellow-900'}`} />
                    系統負載平均值
                  </h4>
                  <div className="grid grid-cols-3 gap-4">
                    {[
                      { label: '1分鐘', value: systemHealth.load_average.one_minute },
                      { label: '5分鐘', value: systemHealth.load_average.five_minute },
                      { label: '15分鐘', value: systemHealth.load_average.fifteen_minute }
                    ].map((load) => {
                      const status = getStatusFromValue(load.value, 'load')
                      const statusColors = getStatusColor(status)
                      
                      return (
                        <div key={load.label} className={`rounded-lg p-3 text-center border border-gray-200 ${isDark ? 'bg-gray-700' : 'bg-white'}`}>
                          <div className={`text-sm mb-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{load.label}</div>
                            <div className={`text-lg font-bold mb-2 ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                            ...
                            <div className={`text-xs px-2 py-1 rounded-full ${statusColors.bgColor} ${statusColors.color}`}>
                              {status === 'good' ? '正常' : status === 'warning' ? '警告' : '超載'}
                            </div>
                        </div>
                      )
                    })}
            ...
            <div className="space-y-6">
              ...
              <div className={`rounded-lg p-4 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                <h4 className={`font-semibold mb-4 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                  <FontAwesomeIcon icon={faMemory} className={`mr-2 ${isDark ? ' text-green-400' : ' text-green-900'}`} />
                  實體記憶體 (RAM)
                </h4>
                  ...
                  <div className="relative w-32 h-32">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        <div className={`text-xl font-bold ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{memoryPercent.toFixed(1)}%</div>
                        <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>已使用</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                    <div className={isDark ? 'text-gray-300' : 'text-gray-600'}>總容量</div>
                    <div className={`font-semibold ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{formatBytes(memoryTotal)}</div>
                  </div>
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                    <div className={isDark ? 'text-gray-300' : 'text-gray-600'}>已使用</div>
                    <div className={`font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{formatBytes(memoryUsed)}</div>
                  </div>
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                    <div className={isDark ? 'text-gray-300' : 'text-gray-600'}>可用</div>
                    <div className={`font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{formatBytes(memoryAvailable)}</div>
                  </div>
                  <div className={`rounded-lg p-3 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                    <div className={isDark ? 'text-gray-300' : 'text-gray-600'}>使用率</div>
              ...
              {swapTotal > 0 && (
                <div className={`rounded-lg p-4 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                  <h4 className={`font-semibold mb-3 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                    <FontAwesomeIcon icon={faHdd} className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-900'}`} />
                    交換空間 (Swap)
                  </h4>
                  
                  <div className="mb-3">
                    <div className="flex justify-between text-sm mb-2">
                      <span className={isDark ? 'text-gray-300' : 'text-gray-600'}>使用情況</span>
                      <span className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{swapPercent.toFixed(1)}%</span>
                    </div>
                    <div className={`w-full rounded-full h-3 ${isDark ? 'bg-gray-500' : 'bg-gray-200'}`}>
                      <div 
                        className={`h-3 rounded-full transition-all duration-300 ${
                          swapPercent < 30 ? 'bg-green-500' :
                          swapPercent < 70 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className={`rounded-lg p-3 border border-gray-200 ${isDark ? 'bg-gray-700' : 'bg-white'}`}>
                      <div className={isDark ? 'text-gray-300' : 'text-gray-600'}>總計</div>
                      <div className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{formatBytes(swapTotal)}</div>
                    </div>
                    <div className={`rounded-lg p-3 border border-gray-200 ${isDark ? 'bg-gray-700' : 'bg-white'}`}>
                      <div className={isDark ? 'text-gray-300' : 'text-gray-600'}>已使用</div>
                      <div className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{formatBytes(swapUsed)}</div>
                    </div>
                  </div>
                </div>
              )}
          ...
          content: (
            <div className="space-y-6">
              ...
              <div className={`rounded-lg p-6 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                <h4 className={`font-semibold mb-4 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                  <FontAwesomeIcon icon={faThermometerHalf} className={`mr-2 ${isDark ? 'text-orange-400' : 'text-orange-900'}`} />
                  當前系統溫度
                </h4>
                ...
                <div className="flex items-center justify-center mb-4">
                  <div className="relative">
                    {/* 溫度計背景 */}
                    <div className={`w-8 h-48 rounded-full relative ${isDark ? 'bg-gray-500' : 'bg-gray-200'}`}>
                      {/* 溫度液體 */}
                      <div 
                        className={`absolute bottom-0 w-full rounded-full transition-all duration-500 ${
                          tempStatus === 'good' ? 'bg-green-500' :
                          tempStatus === 'warning' ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                        style={{ height: `${Math.min((temp / 100) * 100, 100)}%` }}
                      ></div>
                    </div>
                    
                    {/* 溫度刻度 */}
                    <div className={`absolute -right-8 top-0 h-full flex flex-col justify-between text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                      <span>100°C</span>
                      <span>75°C</span>
                      <span>50°C</span>
                      <span>25°C</span>
                  ...
                  <div className="ml-16 text-center">
                    <div className={`text-4xl font-bold mb-2 ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                      {temp.toFixed(1)}°C
                    </div>
                    <div className={`text-sm px-3 py-1 rounded-full ${tempColors.bgColor} ${tempColors.color}`}>
                      {tempStatus === 'good' ? '正常溫度' : 
                       tempStatus === 'warning' ? '溫度偏高' : '溫度過高'}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className={`rounded-lg p-4 ${isDark ? 'bg-slate-600' : 'bg-blue-50'}`}>
                <h4 className={`font-semibold mb-3 flex items-center ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
                  <FontAwesomeIcon icon={faInfoCircle} className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-900'}`} />
                  溫度管理建議
                </h4>
                ...
                <div className={`text-sm space-y-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  {tempStatus === 'good' && (
                    <div className="flex items-start">
                      <FontAwesomeIcon icon={faCheckCircle} className="mr-2 mt-0.5 text-green-600" />
                      <span>系統溫度正常，散熱良好</span>
                    </div>
                  )}
                  ...
                  {tempStatus === 'warning' && (
                    <>
                      <div className="flex items-start">
                        <FontAwesomeIcon icon={faExclamationTriangle} className="mr-2 mt-0.5 text-yellow-600" />
                        <span>系統溫度偏高</span>
                      </div>
                    </>
                  )}
                  ...
                  {tempStatus === 'critical' && (
                    <>
                      <div className="flex items-start">
                        <FontAwesomeIcon icon={faExclamationTriangle} className="mr-2 mt-0.5 text-red-600" />
                        <span>系統溫度過高，立即採取行動</span>
                      </div>
                    </>
                  )}
                </div>
      ...
      default:
        return { title: '', icon: faServer, color: 'text-gray-600', content: <div>無資料</div> }
    }
  }
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* 模態框標題 */}
          <div className={`sticky top-0 border-b border-gray-200 px-6 py-4 rounded-t-xl ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
...
const HealthCard: React.FC<HealthCardProps> = ({ metric, index, systemHealth, onClick }) => {
  ...
  const getDetailedInfo = (): string => {
    ...
    switch (metric.name) {
      case 'CPU 使用率':
        return `處理器負載: ${systemHealth.cpu_details?.cpu_usage.toFixed(1)}%\n狀態: ${metric.status === 'good' ? '正常' : metric.status === 'warning' ? '警告' : '緊急'}`
      case '記憶體使用率':
        return `總記憶體: ${formatBytes(systemHealth.memory_usage.total)}\n已使用: ${formatBytes(systemHealth.memory_usage.used)}\n可用記憶體: ${formatBytes(systemHealth.memory_usage.available)}\n使用率: ${systemHealth.memory_usage.usage_percent.toFixed(1)}%`
      case '系統溫度':
        return `當前溫度: ${systemHealth.temperature.toFixed(1)}°C\n狀態: ${metric.status === 'good' ? '正常運行' : metric.status === 'warning' ? '溫度偏高' : '溫度過高'}`
      default:
  ...
  const hasProgressBar = metric.name.includes('使用率') || metric.name.includes('溫度')
  const isClickable = ['CPU 使用率', '記憶體使用率', '系統溫度'].includes(metric.name)
  ...
  return (
    <motion.div
      ...
      style={{ borderLeftColor: statusColors.dotColor.replace('bg-', '').replace('-500', '') === 'green' ? '#10b981' : 
    ...
    >
        ...
        <div className="flex-1">
          ...
          <p className={`text-2xl font-bold mb-1 ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>
            {typeof metric.value === 'number' ? 
              metric.value.toFixed(metric.name.includes('使用率') || metric.name.includes('溫度') ? 1 : 0) : 
              metric.value
            }
            {metric.unit && <span className={`text-lg ml-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{metric.unit}</span>}
          </p>
          {metric.description && (
          ...
          <div className="flex items-center justify-between mt-2">
            <span className={`text-xs uppercase tracking-wide font-medium ${statusColors.color}`}>
              {metric.status === 'good' ? '正常' :
               ...
               metric.status === 'critical' ? '緊急' : '未知'}
            </span>
          </div>
        </div>
        <div className={`${statusColors.bgColor} p-3 rounded-lg group-hover:scale-110 transition-transform duration-300`}>
          <FontAwesomeIcon 
            icon={metric.icon} 
      ...
      {showTooltip && getDetailedInfo() && !isClickable && (
        <div className={`absolute z-50 bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 text-xs rounded-lg shadow-lg min-w-max max-w-xs ${
...
const LoadAverageDetails: React.FC<LoadAverageProps> = ({ loadAverage }) => {
  ...
  const getLoadDescription = (value: number): string => {
    if (value < 0.5) return '系統空閒'
    if (value < 1.0) return '負載正常'
    if (value < 1.5) return '負載較高'
    if (value < 2.0) return '負載很高'
    return '系統超載'
  }
...
const NetworkStats: React.FC<NetworkStatsProps> = ({ networkStats }) => {
  ...
  const isDark = actualTheme === 'dark';
  const interfaces = [
    { key: 'ingress', name: '入口網路', icon: faArrowDown, color: 'text-blue-600' },
    { key: 'egress', name: '出口網路', icon: faArrowUp, color: 'text-green-600' },
    { key: 'management', name: '管理網路', icon: faDesktop, color: 'text-purple-600' }
  ]
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{iface.name}</span>
                  <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-500'}`}>({stats.interface})</span>
                </div>
                {hasErrors ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <div className={isDark ? 'text-white' : 'text-gray-600'}>接收</div>
                  <div className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{formatBytes(stats.bytes_received)}</div>
                  <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{stats.packets_received.toLocaleString()} 封包</div>
                </div>
                <div>
                  <div className={isDark ? 'text-white' : 'text-gray-600'}>傳送</div>
                  <div className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{formatBytes(stats.bytes_transmitted)}</div>
                  <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{stats.packets_transmitted.toLocaleString()} 封包</div>
                </div>
              </div>
              ...
              {hasErrors && (
                <div className="mt-2 pt-2 border-t border-gray-200">
                  <div className="text-sm text-yellow-600">
                    錯誤: 接收 {stats.errors_received}, 傳送 {stats.errors_transmitted}
                  </div>
                </div>
              )}
            </motion.div>
...
export default function Home() {
  ...
  const healthMetrics = useMemo((): HealthMetric[] => {
    ...
    const metrics: HealthMetric[] = [
      ...
      },
      {
        ...
        color: 'bg-green-500',
        bgColor: 'bg-green-50',
        description: `已用 ${formatBytes(systemHealth.memory_usage.used)} / ${formatBytes(systemHealth.memory_usage.total)}`
      },
      {
        name: '系統溫度',
        value: temperature,
  ...
  const quickNavigation = [
    {
      title: '網路流量監控',
      description: '查看 IPv4/IPv6 流量統計和趨勢分析',
      icon: faChartLine,
      color: 'bg-blue-500',
      href: '/dashboard',
      badge: '即時'
    ...
    {
      title: '存取控制',
      description: '管理 IP 白名單、黑名單和防火牆規則',
      icon: faLock,
      color: 'bg-red-500',
      href: '/access-control',
      badge: '安全'
  ...
  return (
    <>
      <Head>
        <title>NetGuardia - 網路安全監控系統</title>
        <meta name="description" content="NetGuardia 網路安全監控系統主頁" />
      ...
      <Layout>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className={`rounded-lg shadow-md px-3 py-1 flex items-center ${
                ...
                <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {error ? '系統錯誤' : systemHealth ? '系統正常' : '連接中'}
                </span>
              </div>
        ...
        {error && (
          ...
          >
            <div className="flex items-center">
              ...
              <span className="font-medium">系統狀態載入失敗: {error}</span>
            </div>
          </motion.div>
        )}
              ...
              <div className="max-h-60 overflow-y-auto pr-2 space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-white' : 'text-gray-600'}>系統</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{systemHealth.system_info.os_name} {systemHealth.system_info.os_version} ({systemHealth.system_info.architecture}, {systemHealth.system_info.kernel_version})</span>
                  </div>
                  {systemHealth.cpu_details && (
                    <>
                      <div className="flex justify-between">
                        <span className={isDark ? 'text-white' : 'text-gray-600'}>CPU 型號</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{systemHealth.cpu_details.cpu_brand}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={isDark ? 'text-white' : 'text-gray-600'}>CPU 核心</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{systemHealth.cpu_details.core_count}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={isDark ? 'text-white' : 'text-gray-600'}>CPU 頻率</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{systemHealth.cpu_details.cpu_frequency} MHz</span>
                      </div>
                    </>
                  )}
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-white' : 'text-gray-600'}>RAM 大小</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{formatBytes(systemHealth.memory_usage.total)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-white' : 'text-gray-600'}>系統時間</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{new Date(systemHealth.timestamp * 1000).toLocaleString('zh-TW')}</span>
                  </div>
                  {systemHealth.uptime_seconds && (
                    <div className="flex justify-between">
                      <span className={isDark ? 'text-white' : 'text-gray-600'}>運行時間</span>
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>{formatUptimeFromSeconds(systemHealth.uptime_seconds)}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <h2 className={`text-2xl font-bold mb-6 ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>快速導航</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {quickNavigation.map((item, index) => (
              ...
              >
                <Link href={item.href}>
                  <div className={`rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300 border-l-4 border-transparent hover:border-blue-500 ${
                    isDark ? 'bg-gray-600' : 'bg-white'
                  }`}>
